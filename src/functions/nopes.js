...
export default async payload => {
  ...
  return [{
    topic: 'broadcast',
    payload: {
      message: `${messageStart} ${strings.statsHas} ${await reactions(payload.period, payload.filter, payload.room.id, payload.user.id, 'nope')} ${strings.nopesOutro} ${strings.nopesIcon}`
    }
  }]
}
